document.getElementById("meuFormulariog").addEventListener("submit", function (e) {
    const email = document.getElementById("campoEmailg").value.trim().toLowerCase();
    const texto = document.getElementById("campoTextog").value.trim().toLowerCase();
    ...
    const divErro = document.getElementById("divErrog");
    divErro.innerHTML = "";
    ...
    if (noctafly.includes(texto)) {
        divErro.innerHTML += "NocTaFly no está disponble para verificación."
        e.preventDefault();
    }

    if (cyberattack.includes(texto)) {
        divErro.innerHTML += "Cyberattack no está disponble para verificación."
        e.preventDefault();
    }

    if (pou2tiktok.includes(texto)) {
        divErro.innerHTML += "Pou2tiktok no está disponble para verificación."
        e.preventDefault();
    }

    if (meuspous.includes(texto)) {
        divErro.innerHTML += "No te atrevas a poner mis proprios nombres. Tú puedes ser baneado(a)!"
        e.preventDefault();
    }
