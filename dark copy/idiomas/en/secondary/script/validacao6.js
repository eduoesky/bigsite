document.getElementById("meuFormulariog").addEventListener("submit", function (e) {
    ...
    if (noctafly.includes(texto)) {
        divErro.innerHTML += "NocTaFly is not available to be checked."
        e.preventDefault();
    }

    if (cyberattack.includes(texto)) {
        divErro.innerHTML += "Cyberattack is not available to be checked."
        e.preventDefault();
    }

    if (pou2tiktok.includes(texto)) {
        divErro.innerHTML += "Pou2tiktok is not available to be checked."
        e.preventDefault();
    }
