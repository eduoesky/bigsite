document.getElementById("FormularioErro").addEventListener("submit", function (event) {
    ...
    if (campoEmail.length < 15) {
        hasErrors = true;
        divErro.innerHTML = "O email não pode ter menos que 15 caracteres *_*";
    } else if (!emailRe.test(campoEmail)) {
        hasErrors = true;
        divErro.innerHTML = "São permitidos apenas gmails *_*";
        ...
        hasErrors = true;
        divErro.innerHTML = "Não comece o email com símbolos ou números *_*";
        ...
        hasErrors = true;
        divErro.innerHTML = "Não use emojis no email *_*";
    } else if (simbolosRe.test(campoEmail)) {
        hasErrors = true;
        divErro.innerHTML = "Não use símbolos especiais no email *_*";
    }
    if (/(\w)\1{2,}/.test(campoTexto)) {
        hasErrors = true;
        divErro.innerHTML = "Não insira letras repetidas em sequência *_*";
    }

    if (emojiRe.test(campoTexto)) {
        hasErrors = true;
        divErro.innerHTML = "Por favor, digite sem usar emojis *_*";
    } else if (campoTexto.replace(/\s/g, '').length < 25) {
        hasErrors = true;
        divErro.innerHTML = "Relate o erro com mais palavras *_*";
    } else if (campoTexto.includes("@")) {
        hasErrors = true;
        divErro.innerHTML = "Não são permitidos arrobas *_*";
    } else if (campoTexto.includes("*")) {
        hasErrors = true;
        divErro.innerHTML = "Asteriscos não são permitidos *_*";
    }
        for (const palavraProibida of palavroesProibidos) {
            ...
            if (regexPalavraProibida.test(campoTexto)) {
                hasErrors = true;
                divErro.innerHTML = "Não use palavrões *_*";
                break;
            }
        }
